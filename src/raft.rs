...
impl<T: Storage> Raft<T> {
    ...
    pub fn bcast_append(&mut self) {
        ...
        // prs.iter_mut()
        //     .for_each(|(id, pr)| info!(self.logger, "bcast_append prsset_id: {}", *id));
        self.set_prs(prs);
    ...
    pub fn append_entry(&mut self, es: &mut [Entry]) {
        ...
        info!(self.logger, "maybe_update: {} {}", maybe_update_status, self.mut_prs().get_mut(self_id).unwrap().matched);
    ...
    pub fn tick_election(&mut self) -> bool {
        ...
        info!(self.logger, "stepping inside tick_election: {} {} {} {:?}", m.to, m.term, m.index, m.get_msg_type());  //MsgAppend
        let _ = self.step(m);
        true
    }
    ...
    fn tick_heartbeat(&mut self) -> bool {
        ...
        if self.election_elapsed >= self.election_timeout {
            ...
            if self.check_quorum {
                ...
                has_ready = true;
                info!(self.logger, "stepping inside tick_heartbeat 1: {} {} {} {:?}", m.to, m.term, m.index, m.get_msg_type());  //MsgAppend
                let _ = self.step(m);
            }
        ...
        if self.heartbeat_elapsed >= self.heartbeat_timeout {
            ...
            info!(self.logger, "stepping inside tick_heartbeat 2: {} {} {} {:?}", m.to, m.term, m.index, m.get_msg_type());  //MsgAppend
            let _ = self.step(m);
        }
        has_ready
    }
